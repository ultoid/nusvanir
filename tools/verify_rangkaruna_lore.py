"""Validate Rangkaruna fauna narrative/data separation and preservation."""
import hashlib
import json
import re
import sys
...
FOLDER = BEST/'09_Laut_Rangkaruna'
RACES = {}

def main():
    errors = []
    snapshot = read(REPORT/'pre_rangkaruna_narrative.json')
    hashes = read(REPORT/'pre_rangkaruna_hashes.json')
    allowed = {'07_Bestiary/BESTIARY_REWORK_CHANGELOG.md','07_Bestiary/BESTIARY_STANDARD_V2.md',
               ...
               'tools/rework_rangkaruna_lore.py','tools/verify_rangkaruna_lore.py'}
    allowed |= {'tools/verify_agnitra_lore.py','tools/verify_arkananta_chiefs_lore.py','tools/verify_sayendra_sentinels_lore.py',
                'tools/verify_raksamala_commanders_lore.py'}
    changed = []
    ...
    for p in sorted(FOLDER.glob('*.json')):
        ...
        for heading in ['## Deskripsi','## Bentuk','## Tingkah Laku','## Habitat','## Kisah']:
        ...
        approval_path = REPORT/'rangkaruna_lore_approval.json'
        ...
        entries.append({'id':obj['id'],'words':len(text.split()),'runtime_id':enemy['id']})
    if len(entries) != 10:
        errors.append('Expected 10 Rangkaruna fauna pairs')
    result = {'errors':errors,'entries':entries,'changed_baseline_files':changed,
              'scope':'10 narrative/data pairs; identities, original fields, sources, runtime references, and outside-batch preservation',
              'balance_simulation':None,'playtest':None,
              'new_narrative_approval':read(REPORT/'rangkaruna_lore_approval.json')['approval'] if (REPORT/'rangkaruna_lore_approval.json').exists() else None}
    (REPORT/'rangkaruna_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
