"""Validate Agnitra narrative/data separation and preservation against its batch baseline."""
import hashlib
import json
...
def main():
    errors = []
    material_map = {
        'Blackhorn': ('Adamantine Nest', 'Adamantine'),
        'Goldenscale': ('Orichalcum Nest', 'Orichalcum'),
        'Moonfang': ('Silver Nest', 'Silver'),
        'Silverwing': ('Mithril Nest', 'Mithril'),
    }
    snapshot = read(REPORT/'pre_agnitra_narrative.json')
    hashes = read(REPORT/'pre_agnitra_hashes.json')
    ...
    allowed |= {'tools/rework_agnitra_lore.py', 'tools/verify_agnitra_lore.py'}
    changed = []
    ...
    for p in sorted((BEST/'05_Dragons_of_Agnitra_Nest').glob('*.json')):
        obj = read(p)
        expected_nest, expected_material = material_map[p.stem]
        identity = obj.get('lore_identity', {})
        if obj.get('race') != 'Nagarasven':
            errors.append(f'{p.stem}: race must be Nagarasven')
        if identity.get('nest') != expected_nest or identity.get('signature_material') != expected_material:
            errors.append(f'{p.stem}: incorrect nest/material identity')
        ...
        for heading in ['## Deskripsi','## Bentuk','## Kepribadian dan Sikap','## Lokasi Nest','## Kisah']:
        ...
        approval_path = REPORT/'agnitra_lore_approval.json'
    ...
    if len(entries) != 4:
        errors.append('Expected 4 Agnitra pairs')
    result = {'errors':errors,'entries':entries,'changed_baseline_files':changed,
              ...
              'new_narrative_approval':read(REPORT/'agnitra_lore_approval.json')['approval'] if (REPORT/'agnitra_lore_approval.json').exists() else None}
    (REPORT/'agnitra_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
