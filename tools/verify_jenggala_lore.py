"""Validate Jenggala narrative/data separation and preservation against its batch baseline."""
import hashlib
import json
...
def main():
    errors = []
    snapshot = read(REPORT/'pre_jenggala_narrative.json')
    hashes = read(REPORT/'pre_jenggala_hashes.json')
    ...
    for name, old_hash in hashes.items():
        ...
        if not p.exists() or hashlib.sha256(p.read_bytes()).hexdigest() != old_hash:
            changed.append(name)
            if not name.startswith('07_Bestiary/02_Jenggala/') and name not in allowed:
    ...
    for p in sorted((BEST/'02_Jenggala').glob('*.json')):
        obj = read(p)
        ...
        approval_path = REPORT/'jenggala_lore_approval.json'
    ...
    if len(entries) != 15:
        errors.append('Expected 15 Jenggala pairs')
    result = {'errors':errors,'entries':entries,'changed_baseline_files':changed,
              'scope':'15 narrative/data pairs; original fields, sources, runtime references, and outside-batch preservation',
              'balance_simulation':None,'playtest':None,
              'new_narrative_approval':read(REPORT/'jenggala_lore_approval.json')['approval'] if (REPORT/'jenggala_lore_approval.json').exists() else None}
    (REPORT/'jenggala_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
