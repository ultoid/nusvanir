"""Validate Nusa Sayendra sentinels narrative/data separation and preservation."""
import hashlib
import json
import re
import sys
...
FOLDER = BEST/'07_Sentinels_of_Nusa_Sayendra'
RACES = {'Jenderal_Jatayu':'Garuda','Jenderal_Paksi':'Garuda','Jenderal_Sempati':'Garuda','Jenderal_Suparna':'Garuda'}

def main():
    errors = []
    snapshot = read(REPORT/'pre_sayendra_sentinels_narrative.json')
    hashes = read(REPORT/'pre_sayendra_sentinels_hashes.json')
    allowed = {'07_Bestiary/BESTIARY_REWORK_CHANGELOG.md','07_Bestiary/BESTIARY_STANDARD_V2.md',
               ...
               'tools/rework_sayendra_sentinels_lore.py','tools/verify_sayendra_sentinels_lore.py'}
    ...
    for p in sorted(FOLDER.glob('*.json')):
        ...
        for heading in ['## Deskripsi','## Bentuk','## Kepribadian dan Sikap','## Kedudukan dan Kuil Penjagaan','## Kisah']:
        ...
        approval_path = REPORT/'sayendra_sentinels_lore_approval.json'
    ...
    if len(entries) != 4:
        errors.append('Expected 4 Nusa Sayendra sentinel pairs')
    result = {'errors':errors,'entries':entries,'changed_baseline_files':changed,
              'scope':'4 narrative/data pairs; identities, original fields, sources, runtime references, and outside-batch preservation',
              'balance_simulation':None,'playtest':None,
              'new_narrative_approval':read(REPORT/'sayendra_sentinels_lore_approval.json')['approval'] if (REPORT/'sayendra_sentinels_lore_approval.json').exists() else None}
    (REPORT/'sayendra_sentinels_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
