"""Validate Arkananta chiefs narrative/data separation and preservation."""
import hashlib
import json
import re
import sys
...
RACES = {'Grok_Tulang_Besi':'Troliogoro','Krom_Batugilang':'Butoraksa','Nyai_Larasati':'Rakshorien'}

def main():
    errors = []
    snapshot = read(REPORT/'pre_arkananta_chiefs_narrative.json')
    hashes = read(REPORT/'pre_arkananta_chiefs_hashes.json')
    allowed = {'07_Bestiary/BESTIARY_REWORK_CHANGELOG.md','07_Bestiary/BESTIARY_STANDARD_V2.md',
               ...
               'tools/rework_arkananta_chiefs_lore.py','tools/verify_arkananta_chiefs_lore.py'}
    allowed |= {'tools/verify_agnitra_lore.py'}
    changed = []
    ...
    for p in sorted(FOLDER.glob('*.json')):
        ...
        for heading in ['## Deskripsi','## Bentuk','## Kepribadian dan Sikap','## Kedudukan dan Wilayah','## Kisah']:
        ...
        approval_path = REPORT/'arkananta_chiefs_lore_approval.json'
    ...
    if len(entries) != 3:
        errors.append('Expected 3 Arkananta chief pairs')
    result = {'errors':errors,'entries':entries,'changed_baseline_files':changed,
              'scope':'3 narrative/data pairs; identities, original fields, sources, runtime references, and outside-batch preservation',
              'balance_simulation':None,'playtest':None,
              'new_narrative_approval':read(REPORT/'arkananta_chiefs_lore_approval.json')['approval'] if (REPORT/'arkananta_chiefs_lore_approval.json').exists() else None}
    (REPORT/'arkananta_chiefs_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
