"""Validate Thalantira narrative/data separation and preservation against its batch baseline."""
import hashlib
import json
...
def main():
    errors = []
    snapshot = read(REPORT/'pre_thalantira_narrative.json')
    hashes = read(REPORT/'pre_thalantira_hashes.json')
    ...
    allowed.add('tools/verify_thalantira_lore.py')
    # Later Avalerion batch has its own archive and approval record.
    if (REPORT/'avalerion_lore_approval.json').exists():
    ...
    for name, old_hash in hashes.items():
        ...
        if not p.exists() or hashlib.sha256(p.read_bytes()).hexdigest() != old_hash:
            changed.append(name)
            if not name.startswith('07_Bestiary/03_Thalantira/') and name not in allowed:
    ...
    for p in sorted((BEST/'03_Thalantira').glob('*.json')):
        obj = read(p)
        ...
        approval_path = REPORT/'thalantira_lore_approval.json'
    ...
    if len(entries) != 18:
        errors.append('Expected 18 Thalantira pairs')
    result = {'errors':errors,'entries':entries,'changed_baseline_files':changed,
              'scope':'18 narrative/data pairs; original fields, sources, runtime references, and outside-batch preservation',
              'balance_simulation':None,'playtest':None,
              'new_narrative_approval':read(REPORT/'thalantira_lore_approval.json')['approval'] if (REPORT/'thalantira_lore_approval.json').exists() else None}
    (REPORT/'thalantira_verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
